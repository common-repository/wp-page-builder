...
if (class_exists('Zigapage_b_notice')) {
   new Zigapage_b_notice();
    return;
}
 
class Zigapage_b_notice {
	...
	add_action( 'wp_ajax_zgpb_f_notice_dismiss', array( $this, 'notice_dismiss' ) );
        add_action( 'wp_ajax_zgpb_f_notice_rated', array( $this, 'notice_rated' ) );
    ...
    public function notice_rated() {
            ...
            $data              = get_option( 'zigapage_f_notice_1', array() );
            $data['time'] 	 = time();
            $data['dismissed'] = true;
            $data['rated'] = true;

            update_option( 'zigapage_f_notice_1', $data );
            die();
    }
    ...
    public function notice_add() {
        ...
        $data = get_option('zigapage_f_notice_1');
        $time = time();
        $load = false;
        ...
        if(ZGPBLD_LITE){
                // after 7 days, add the message
                $notice_url = 'https://wordpress.org/support/plugin/wp-page-builder/reviews/?filter=5#new-post';
                $notice_heading = esc_html__( "Thanks for using Landera!", "FRocket_admin" );
                $notice_content = __( 'Hey, I noticed you used Landera - that’s awesome! Could you please do me a BIG favor and give it a 5-star rating on WordPress to help us spread the word and boost our motivation?', 'FRocket_admin' );
                ...
                $notice_heading = esc_html__( "Thanks for using Landera!", "FRocket_admin" );
                $notice_content = __( 'Hey, I noticed you used Landera - that’s awesome! Could you please do me a BIG favor and give it a 5-star rating on Codencayon to help us spread the word and boost our motivation?', 'FRocket_admin' );
                $button_content1 = esc_html__( "Ok. you deserve it", "FRocket_admin" );
                $button_content2 = esc_html__( "Nope, maybe later", "FRocket_admin" );
                $button_content3 = esc_html__( "I already did", "FRocket_admin" );
            }
        ...
        <div class="notice zgpb-ext-notice" style="display: none;">
                ...
                <div
                        class="zgpb-ext-notice-message">
                        <strong><?php echo $notice_heading; ?></strong>
                        <?php echo $notice_content; ?>
                </div>
                <div class="zgpb-ext-notice-cta">
                        <a href="<?php echo esc_url( $notice_url ); ?>" class="zgpb-ext-notice2-act button-primary" target="_blank">
                        <?php echo $button_content1; ?>
                        </a>
                        <button class="zgpb-ext-notice2-act2 button" 
                                data-msg="<?php esc_html_e( 'Saving', 'FRocket_admin'); ?>">
                                    <?php echo $button_content2; ?>
                        </button>
                        <button class="zgpb-ext-notice-dismiss zgpb-dismiss-rated" 
                                data-msg="<?php esc_html_e( 'Saving', 'FRocket_admin'); ?>">
    ...
    public function notice_footer( $text ) {
            ...
            if ( (!empty( $current_screen->id ) && strpos( $current_screen->id, 'zgpb_page_builder' ) !== false )
                 || ( !empty( $current_screen->id ) && strpos( $current_screen->id, 'zigapage-builder-' ) !== false    )
                    ) {
                    $url  = 'https://wordpress.org/support/plugin/wp-page-builder/reviews/?filter=5#new-post';
                    $text = sprintf( __( 'Please rate <strong>Landera</strong> <a href="%s" target="_blank" rel="noopener" >&#9733;&#9733;&#9733;&#9733;&#9733;</a> on <a href="%s" target="_blank">WordPress.org</a> to help us spread the word. Thank you from the Landera team!', 'zgpbd_admin' ), $url, $url );
            }
            return $text;
    }
