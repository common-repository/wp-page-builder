...
+function ($) { "use strict";
    ...
    Iconpicker.prototype.createButtonBar = function(){
        ...
        for(var i = 0; i < op.cols; i++){
            var btn = $('<button class="sfdc-btn sfdc-btn-primary"><span class="sfdc-glyphicon"></span></button>');
            var td = $('<td class="text-center"></td>');
            if(i == 0 || i == op.cols - 1){
                btn.val((i==0) ? -1: 1);
                btn.addClass((i==0) ? 'sfdc-btn-previous': 'sfdc-btn-next');
                btn.find('span').addClass( (i == 0) ? 'sfdc-glyphicon-arrow-left': 'sfdc-glyphicon-arrow-right');
                td.append(btn);
                tr.append(td);
            }
    ...
    Iconpicker.prototype.updateButtonBar = function(page){
        ...
        var btn_prev = op.table.find('.sfdc-btn-previous');
        var btn_next = op.table.find('.sfdc-btn-next');
        (page == 1) ? btn_prev.addClass('sfdc-disabled'): btn_prev.removeClass('sfdc-disabled');
        (page == total_pages) ? btn_next.addClass('sfdc-disabled'): btn_next.removeClass('sfdc-disabled');
    };

    Iconpicker.prototype.bindEvents = function(){
        var op = this.options;
        var el = this;
        op.table.find('.sfdc-btn-previous, .sfdc-btn-next').off('click').on('click', function(){
            var inc = parseInt($(this).val());
            el.changeList(op.page + inc);
        });
        op.table.find('.sfdc-btn-icon').off('click').on('click', function(){
            el.select($(this).val());
    ...
    Iconpicker.prototype.select = function(icon){
        ...
        if(icon != '' && op.selected >= 0){
            ...
            op.table.find('button.sfdc-btn-warning').removeClass('sfdc-btn-warning');
        }
    };
    ...
    Iconpicker.prototype.switchPage = function(icon){
        ...
        op.table.find('.'+icon).parent().addClass('sfdc-btn-warning');
    };
    ...
    Iconpicker.prototype.changeList = function(page){
        ...
        for(var i = 0; i < op.rows; i++){
            ...
            for(var j = 0; j < op.cols; j++){
                ...
                var btn = $('<button class="sfdc-btn sfdc-btn-default sfdc-btn-icon"></button>').hide();
                if(pos < op.icons.length){
                    var v = op.iconClassFix + op.icons[pos];
                    btn = $('<button class="sfdc-btn sfdc-btn-default sfdc-btn-icon" value="' + v + '" title="' + v + '"><i class="' + op.iconClass + ' ' + v + '"></i></button>');
                }
        ...
        return this.each(function () {
            ...
            if (typeof option == 'string') data[option](params)
                ...
                $this.on('click', function(e){
                    ...
                    $this.data('bs.sfdc-popover').tip().addClass('iconpicker-popover')
                    $this.popover('show');
                });
                data.select(op.icon);
            }
